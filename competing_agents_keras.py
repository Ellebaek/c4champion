    ...
    for i in range(num_games):
        ...
        while j < 50 and np.sum(open_actions(g)) > 0 and not d:
            ...
            if randval > e2:
                ...
                # a = rand_index_filter(filter)
            if agent1s_turn:
                allQ = agent1.model.predict(s)
                a = best_allowed_action(allQ, filter, top)
            else:
                allQ = agent2.model.predict(s)
                a = best_allowed_action(allQ, filter, top)
...
def duel_and_save_games(agent1, agent2, duelname):
    agent1_to_start = False
    wList = []
    for i in range(2):
        bList = []
...
train_agent_against_list(CHAMPION, [])
e_init = 0.5
...
for i in range(1, num_generations + 1, 1):
    ...
    while score < 10 and episode_count < max_num_episodes:
        train_agent_against_list(CHALLENGER,  opponent_list, episode_count)
...
duel_result = duel_and_save_games(challenger_list[final_challenger_id], CHAMPION, "duel_{0}x{1}".format(num_generations,num_episodes))
